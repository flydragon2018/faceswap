...
def get_face68_txt(face_img): 
    ...
    for i in range(len(shape_np)):
        ...
        file.write('{0} {1}\n'.format(shape_np[i][0],shape_np[i][1]))      
    file.close()
...
if __name__ == '__main__' :
    ...
    #ilename2 = 'donald_trump.jpg'
    
    filename1 = 'source1.jpg'
    filename2 = 'source2.jpg'

    get_face68_txt(filename1)

    get_face68_txt(filename2)
    ...
    #modify window to adapt too big file

    cv2.imwrite( '{0}_{1}_swap.jpg'.format(filename1,filename2),output)

    width, height=output.shape[:2]
    cv2.namedWindow('Face Swapped', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Face Swapped', width, height)
